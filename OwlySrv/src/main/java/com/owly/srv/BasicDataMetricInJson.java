...
public class BasicDataMetricInJson {
	...
	public void interpolateDataMetrics(Long repeatInterval,Float defaultValue){
		
		//long interpolated;
		//long oldDate;	
		
		JSONArray newjsonDataArray = new JSONArray();
		JSONArray oldjsonDataArray = new JSONArray();
		
		// The the info we have in this object related to metrics
		oldjsonDataArray = this.jsonDataArray;
		
		// Number of elements in the array
		int nbElements = oldjsonDataArray.size(); 
		logger.debug("nbElements = " + nbElements);

		if (nbElements>0){
			
			//Initialize first value of the interpolation
			newjsonDataArray.add((JSONArray) oldjsonDataArray.get(0));
			for (int i = 1; i < nbElements; i++) {
								
				// Obtain data related to date for actual index and prvious one
				Long lastDate= (Long) ((JSONArray) oldjsonDataArray.get(i-1)).get(0);
				Long actualDate = (Long) ((JSONArray) oldjsonDataArray.get(i)).get(0);
				Float actualValue =  (Float) ((JSONArray) oldjsonDataArray.get(i)).get(1);
				
				//logger.debug("Lastdate = " + lastDate + " and new date = "+actualDate+";Difference = " + (actualDate-lastDate));
				if (actualDate < (lastDate - 5*(repeatInterval))){
					//logger.debug("We have a big gap we need to add samples");
					//We need to add metrics for the gap discovered
					long interpolated = lastDate-repeatInterval;
					long oldDate=actualDate;					
					while (interpolated > oldDate){
						//logger.debug("Interporlated  date = " + interpolated + " with value = "+ defaultValue );
						interpolated = interpolated - repeatInterval;

						//Add this value into the Metric data.
						JSONArray jsonArray = new JSONArray();
						jsonArray.add(interpolated);
						jsonArray.add(defaultValue);
						newjsonDataArray.add(jsonArray);
					}
				}
				else{
					// In this case we don´t have to do time interpolation.
					JSONArray jsonArray = new JSONArray();
					jsonArray.add(actualDate);
					jsonArray.add(actualValue);
					newjsonDataArray.add(jsonArray);
					
				}
			}
			logger.info("Original number Elements = " + nbElements+";Interpolated elements = " + newjsonDataArray.size());
		}else
		{
			// No data to process
			newjsonDataArray=oldjsonDataArray;
		}
		
		//Clean the data till now inserted.
				this.jsonDataArray.clear();
				this.jsonDataArray = newjsonDataArray;
